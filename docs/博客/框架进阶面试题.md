...
**Chrome浏览器的架构图**： ![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/83e928a4f95d4ebe91d0881b5b2cf94b~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp) 从图中可以看出，最新的 Chrome 浏览器包括：
...
浏览器的渲染进程的线程总共有五种： ![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/d6e583f59dc742b9b4e88cf3a3b0f1d4~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp) **（1）GUI渲染线程** 负责渲染浏览器页面，解析HTML、CSS，构建DOM树、构建CSSOM树、构建渲染树和绘制页面；当界面需要**重绘**或由于某种操作引发**回流**时，该线程就会执行。
...
```javascript
javascript 体验AI代码助手 代码解读复制代码// index.js
...
![业务流程图1.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/3f6837d8d1c74cf2894d8967a20115d9~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp) 很多网站的资源后面都加了版本号，这样做的目的是：每次升级了 JS 或 CSS 文件后，为了防止浏览器进行缓存，强制改变版本号，客户端浏览器就会重新下载新的 JS 或 CSS 文件 ，以保证用户能够及时获得网站的最新更新。
...
大致过程如图所示： ![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/4d69da20c3f84782948226798effc60a~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp)
...
```javascript
javascript 体验AI代码助手 代码解读复制代码// 保存数据到 localStorage
...
```javascript
javascript 体验AI代码助手 代码解读复制代码// 保存数据到 sessionStorage
...
```javascript
javascript 体验AI代码助手 代码解读复制代码Access-Control-Allow-Origin: http://api.bob.com  // 和Orign一直
...
```javascript
javascript 体验AI代码助手 代码解读复制代码Access-Control-Allow-Origin: http://api.bob.com  // 允许跨域的源地址
...
```javascript
javascript 体验AI代码助手 代码解读复制代码'Access-Control-Allow-Origin'  
...
```javascript
javascript 体验AI代码助手 代码解读复制代码// 原生 xml 的设置方式
...
```javascript
javascript 体验AI代码助手 代码解读复制代码<script>
...
```javascript
javascript 体验AI代码助手 代码解读复制代码this.$http = axios;
...
```javascript
javascript 体验AI代码助手 代码解读复制代码var querystring = require('querystring');
...
```javascript
javascript 体验AI代码助手 代码解读复制代码<iframe id="iframe" src="http://www.domain2.com/b.html" style="display:none;"></iframe>
<script>       
...
```http
http 体验AI代码助手 代码解读复制代码location / {
...
```javascript
javascript 体验AI代码助手 代码解读复制代码#proxy服务器
server {
    listen       81;
...
```javascript
javascript 体验AI代码助手 代码解读复制代码var xhr = new XMLHttpRequest();
// 前端开关：浏览器是否读写cookie
xhr.withCredentials = true;
...
```javascript
javascript 体验AI代码助手 代码解读复制代码var express = require('express');
...
```javascript
javascript 体验AI代码助手 代码解读复制代码<iframe id="iframe" src="http://child.domain.com/b.html"></iframe>
<script>
...
```ini
ini 体验AI代码助手 代码解读复制代码<iframe id="iframe" src="http://www.domain1.com/c.html" style="display:none;"></iframe>
<script>
    var iframe = document.getElementById('iframe');
...
```javascript
javascript 体验AI代码助手 代码解读复制代码var proxy = function(url, callback) {
    var state = 0;
...
```javascript
javascript 体验AI代码助手 代码解读复制代码<div>user input：<input type="text"></div>
...
```javascript
javascript 体验AI代码助手 代码解读复制代码var http = require('http');
...
两者区别如图示： ![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/97e92248f2654185808f80b65f3ca0f6~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp) 正向代理和反向代理的结构是一样的，都是 client-proxy-server 的结构，它们主要的区别就在于中间这个 proxy 是哪一方设置的。在正向代理中，proxy 是 client 设置的，用来隐藏 client；而在反向代理中，proxy 是 server 设置的，用来隐藏 server。
...
```html
html 体验AI代码助手 代码解读复制代码<ul id="list">
  <li>item 1</li>
  <li>item 2</li>
  <li>item 3</li>
  ......
...
```javascript
javascript 体验AI代码助手 代码解读复制代码// 来实现把 #list 下的 li 元素的事件代理委托到它的父层元素也就是 #list 上：
// 给父层元素绑定事件
...
```javascript
javascript 体验AI代码助手 代码解读复制代码document.addEventListener("click", function(e) {
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/426a3bf11f404821abb636690119621b~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp)
...
Node 的 Event Loop 分为 6 个阶段，它们会按照**顺序**反复运行。每当进入某一个阶段的时候，都会从对应的回调队列中取出函数去执行。当队列为空或者执行的回调函数数量到达系统设定的阈值，就会进入下一阶段。 ![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/ed6d249f54e242fe9dc267f03fa156d3~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp)
...
javascript
 ...
 体验AI代码助手
 代码解读
复制代码setTimeout(() => {    console.log('setTimeout')}, 0)setImmediate(() => {    console.log('setImmediate')})
```
...
```javascript
javascript 体验AI代码助手 代码解读复制代码setTimeout(() => {
  console.log('timer21')
}, 0)
...
```javascript
javascript 体验AI代码助手 代码解读复制代码// 以下会先打印冒泡然后是捕获
...
```javascript
javascript 体验AI代码助手 代码解读复制代码node.addEventListener(
  'click',
  event => {
...
```javascript
javascript 体验AI代码助手 代码解读复制代码enum AllocationSpace {
